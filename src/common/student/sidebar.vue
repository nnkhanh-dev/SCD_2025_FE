...
// Giả sử đường dẫn service này đúng trong dự án của bạn, nếu chưa có hãy tạo file dummy hoặc xóa dòng này
...
const handleClick = async (item) => {
  ...
  if (item.action === "logout") {
    ...
    setTimeout(async () => {
      // Kiểm tra nếu hàm logoutUser tồn tại (đề phòng lỗi nếu chưa import đúng)
      if (typeof logoutUser === "function") {
        await logoutUser();
      } else {
        // Fallback nếu chưa có service
        router.push("/login");
      }
    }, 1500);
  } else {
...
const currentUser = {
  name: "Sarah Johnson",
  role: "Sinh viên IT",
  avatar: "https://i.pravatar.cc/150?img=5", // Ảnh avatar từ layout cũ
};

// --- LOGIC WATCH ACTIVE STATE ---
watchEffect(() => {
...
<style scoped>
/* Scrollbar ẩn nhưng vẫn scroll được, hover mới hiện */

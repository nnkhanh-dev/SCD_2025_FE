...
const fetchInvitations = async () => {
  try {
    ...
    const response = await companyService.getSentInvitations();
...
const handleDelete = (invitation) => {
  if (invitation.status !== "Pending") {
    showAlert("Không thể xóa", "Chỉ có thể xóa lời mời đang chờ phản hồi", "warning");
    return;
  }

  showConfirm(
    "Xác nhận xóa lời mời",
    `Bạn có chắc muốn xóa lời mời gửi cho "${invitation.studentName}"?`,
    async () => {
      try {
        isDeleting.value = true;
        await companyService.deleteInvitation(invitation.id);
        toast.success("Đã xóa lời mời thành công");
        ...
        showAlert(
          "Lỗi xóa lời mời",
          error.response?.data?.message || "Không thể xóa lời mời",
          "error"
        );
      } finally {
          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-xs md:text-sm text-gray-500 mt-1 hidden sm:block">
                Theo dõi lời mời phỏng vấn đã gửi cho ứng viên
              </p>
            </div>
          </div>
        </div>
      ...
      >
        ...
        >
          ...
          <p class="text-lg font-semibold text-gray-600 mb-2">
            {{ selectedStatus === 'all' ? 'Chưa gửi lời mời nào' : 'Không có lời mời phù hợp với bộ lọc' }}
          </p>
          <p class="text-sm text-gray-400">
            {{ selectedStatus === 'all' ? 'Các lời mời phỏng vấn bạn gửi sẽ xuất hiện ở đây' : 'Thử chọn bộ lọc khác' }}
          </p>
        </div>
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-start gap-4">
                  <div
                    class="w-14 h-14 bg-gradient-to-br from-indigo-500 to-blue-600 rounded-xl flex items-center justify-center text-white font-bold text-xl shadow-md shrink-0"
                  >
                    {{ inv.studentName?.charAt(0) || "S" }}
                  </div>
                  <div class="flex-1 min-w-0">
                    <h3
                      class="text-lg font-bold text-gray-900 group-hover:text-indigo-600 transition-colors mb-1"
                    >
                      {{ inv.studentName }}
                    </h3>
                    <p class="text-sm text-gray-600 mb-2">
                      Lời mời cho vị trí: <span class="font-semibold">{{ inv.jobTitle }}</span>
                    </p>
                    <div class="flex flex-wrap gap-3 text-sm text-gray-500">
                      <span class="flex items-center gap-1">
                        ...
                        Gửi lời mời: {{ formatDate(inv.createdAt) }}
                      </span>
                      <span
      ...
      >
        ...
        >
          ...
          <div class="p-6 space-y-6 max-h-[70vh] overflow-y-auto">
            ...
            <div class="flex items-start gap-4">
              <div
                class="w-16 h-16 bg-gradient-to-br from-indigo-500 to-blue-600 rounded-xl flex items-center justify-center text-white font-bold text-2xl shadow-md"
              >
                {{ selectedInvitation.studentName?.charAt(0) || "S" }}
              </div>
              <div class="flex-1">
                <h4 class="text-lg font-bold text-gray-900">
                  {{ selectedInvitation.studentName }}
                </h4>
                <p class="text-gray-600">Vị trí: {{ selectedInvitation.jobTitle }}</p>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="space-y-3">
                <div class="flex items-start gap-3">
                  ...
                  <div>
                    <p class="font-medium text-gray-900">Gửi lời mời phỏng vấn</p>
                ...
                >
                  ...
                  <div>
                    <p class="font-medium text-gray-900">
                      {{ selectedInvitation.status === 'Accepted' ? 'Ứng viên chấp nhận' : 'Ứng viên từ chối' }}
                    </p>
          ...
          <div class="px-6 py-4 border-t border-gray-100 bg-gray-50 flex gap-3">
            <button
              ...
              @click="handleDelete(selectedInvitation)"
              :disabled="isDeleting"
